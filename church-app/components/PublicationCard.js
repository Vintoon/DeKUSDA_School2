...
import { HiDownload, HiPlay, HiBookOpen, HiCalendar, HiHeart, HiChat } from 'react-icons/hi'
...
export default function PublicationCard({ pub }) {
  ...
  return (
    <article className="pub-card bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm flex flex-col">
      <Link href={`/publication/${pub.id}`} className="block relative h-44 bg-gradient-to-br from-brand-800 to-brand-600 overflow-hidden flex-shrink-0">
        {pub.cover_image_url ? (
        ...
        {pub.featured && (
          <div className="absolute top-2 left-2 bg-gold-400 text-white text-xs font-ui font-bold px-2 py-0.5 rounded-full shadow-sm">★ Featured</div>
        )}
