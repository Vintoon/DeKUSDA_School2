...
import { HiSearch, HiFilter, HiChevronRight } from 'react-icons/hi'
...
export default function Home({ user, profile }) {
  ...
  async function loadStats() {
    ...
    setStats({
      ...
      contributors: new Set(data.map(p => p.author_id)).size,
    })
  }
    ...
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <div className="flex gap-4 flex-wrap">
                <a href="#publications"
                  className="px-6 py-3 bg-white text-brand-800 font-ui font-bold rounded-xl hover:bg-blue-50 transition-all shadow-lg">
                  Browse Publications
                </a>
                <a href="/submit"
                  className="px-6 py-3 bg-gold-500 hover:bg-gold-600 text-white font-ui font-bold rounded-xl transition-all shadow-lg">
                  Submit an Article
                </a>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-6">
              ...
              <div className="bg-white/10 border border-white/20 backdrop-blur rounded-2xl p-6 max-w-sm text-center">
                <p className="font-body text-lg italic text-white/90 leading-relaxed mb-2">
                  "{verse.text}"
                </p>
      ...
      <div className="bg-brand-800 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-wrap gap-8 justify-center sm:justify-start">
          {[
            { label: 'Publications', val: stats.total },
            { label: 'Sermons', val: stats.sermons },
            { label: 'Contributors', val: stats.contributors },
          ].map(s => (
        ...
        <div className="grid lg:grid-cols-[1fr_300px] gap-10">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl font-ui text-sm bg-white focus:border-brand-400"
                />
              </div>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="font-ui text-slate-400">
                  {search ? 'Try a different search term.' : 'Be the first to submit one!'}
                </p>
                <a href="/submit" className="inline-block mt-6 px-6 py-2.5 bg-brand-600 text-white rounded-xl font-ui font-semibold text-sm">
                  Submit Article
                </a>
              </div>
            ) : (
              <div className="grid sm:grid-cols-2 gap-6">
                {filtered.map(pub => (
                  <PublicationCard key={pub.id} pub={pub} onClick={setSelectedPub} />
                ))}
              </div>
            )}
          </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-brand-600 to-brand-800 rounded-2xl p-6 text-white text-center">
              ...
              <p className="font-ui text-sm text-blue-200 mb-4">Submit your sermon, testimony, or devotional to be shared with the congregation.</p>
              <a href="/submit"
                className="block px-5 py-2.5 bg-white text-brand-700 rounded-xl font-ui font-bold text-sm hover:bg-blue-50 transition-colors">
                Submit Now
              </a>
            </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select onChange={e => document.documentElement.style.setProperty('--font-body', e.target.value)}
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 font-ui text-sm">
                  ...
                  <div className="flex gap-2">
                    {['sm', 'base', 'lg', 'xl'].map((s, i) => (
                      <button key={s} onClick={() => {
                        const sizes = ['15px','17px','19px','22px']
      ...
      {/* Article modal */}
      {selectedPub && <ArticleModal pub={selectedPub} onClose={() => setSelectedPub(null)} />}
